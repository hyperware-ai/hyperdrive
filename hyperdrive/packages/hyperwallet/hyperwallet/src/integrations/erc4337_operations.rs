use hyperware_process_lib::hyperwallet_client::types::{
    ...
    GetUserOperationReceiptRequest, HyperwalletResponse, HyperwalletResponseData, OperationError,
    PaymasterConfig, SessionId, SubmitUserOperationRequest, SubmitUserOperationResponse,
...
impl OperationResponse {
    ...
    pub fn from_hyperwallet_response(
        response: hyperware_process_lib::hyperwallet_client::types::HyperwalletResponse,
    ) -> Self {
        if response.success {
...
use hyperware_process_lib::eth::{
    Address as EthAddress, TransactionInput, TransactionRequest, U256,
};
...
pub fn build_and_sign_user_operation_for_payment(
    ...
    let call_data_bytes = match parse_hex(&request.call_data) {
        ...
        Err(e) => {
            return HyperwalletResponse::error(
                e.error
                    .unwrap_or_else(|| OperationError::invalid_params("Failed to parse call data")),
            )
        }
    };
    ...
    let paymaster_and_data = if use_paymaster {
        ...
        match build_circle_paymaster_data(config) {
            Ok(data) => data,
            Err(e) => {
                return HyperwalletResponse::error(e.error.unwrap_or_else(|| {
                    OperationError::invalid_params("Failed to build paymaster data")
                }))
            }
        }
    } else {
    ...
    let wallet_info = match state.get_wallet(address, &request.eoa_wallet_id) {
        Some(info) => info,
        None => {
            return HyperwalletResponse::error(OperationError::invalid_params("Wallet not found"))
        }
    };
    ...
    let estimation_request = match build_estimation_request(
        ...
        Err(e) => {
            return HyperwalletResponse::error(e.error.unwrap_or_else(|| {
                OperationError::invalid_params("Failed to build estimation request")
            }))
        }
    };
    ...
    pre_verification_gas = gas_estimates
    ...
    let signature = match sign_user_op(&provider, &user_op, entry_point, &signer) {
        ...
        Err(e) => {
            return HyperwalletResponse::error(
                e.error
                    .unwrap_or_else(|| OperationError::internal_error("Failed to sign UserOp")),
            )
        }
    };
    ...
    info!(
        "Built and signed UserOperation for payment from TBA {}",
        signed_user_op.sender
    );
    ...
    let response_data = BuildAndSignUserOperationResponse {
        signed_user_operation: signed_user_op_json.to_string(),
        entry_point: entry_point.to_string(),
        ready_to_submit: true,
    };

    HyperwalletResponse::success(
        HyperwalletResponseData::BuildAndSignUserOperationForPayment(response_data),
    )
}

fn parse_hex(hex: &str) -> Result<Vec<u8>, OperationResponse> {
    hex::decode(hex.trim_start_matches("0x"))
        .map_err(|_| OperationResponse::error(OperationError::invalid_params("Invalid hex string")))
}
...
) -> Result<U256, OperationResponse> {
    fetch_nonce_from_entry_point(provider, sender, entry_point).map_err(|e| {
        OperationResponse::error(OperationError::internal_error(&format!(
            "Failed to fetch nonce: {}",
            e
        )))
    })
...
fn parse_u256(hex: &str) -> Result<U256, OperationResponse> {
    U256::from_str_radix(hex.trim_start_matches("0x"), 16).map_err(|_| {
        OperationResponse::error(OperationError::invalid_params("Invalid U256 hex string"))
    })
}
...
fn build_estimation_request(
    ...
    let signature = match signer.sign_hash(&hash) {
        Ok(sig) => sig,
        Err(e) => {
            return Err(OperationResponse::error(OperationError::internal_error(
...
fn sign_user_op(
    ...
    let hash = get_user_op_hash(provider, user_op, entry_point).map_err(|e| {
        OperationResponse::error(OperationError::internal_error(&format!(
            "Failed to get UserOp hash: {}",
            e
        )))
    })?;
    signer.sign_hash(&hash).map_err(|e| {
        OperationResponse::error(OperationError::internal_error(&format!(
...
pub fn submit_user_operation(
    ...
    // Parse the signed user operation from string to JSON Value
    let user_op_json = match serde_json::from_str(&req.signed_user_operation) {
        Ok(json) => json,
        Err(e) => {
            return HyperwalletResponse::error(OperationError::invalid_params(&format!(
                "Invalid JSON in signed_user_operation: {}",
                e
            )));
        }
    };

    match bundler::submit_user_operation(user_op_json, req.entry_point.clone()) {
        Ok(user_op_hash) => {
            info!("UserOperation submitted successfully: {}", user_op_hash);
            HyperwalletResponse::success(HyperwalletResponseData::SubmitUserOperation(
                SubmitUserOperationResponse { user_op_hash },
            ))
        }
        Err(e) => {
...
pub fn get_user_operation_receipt(
    ...
    match bundler::get_user_operation_receipt(req.user_op_hash.clone()) {
        Ok(receipt_data) => {
            ...
            HyperwalletResponse::success(HyperwalletResponseData::GetUserOperationReceipt(
                UserOperationReceiptResponse {
                    user_op_hash: req.user_op_hash.clone(),
                    status: "success".to_string(),
                    receipt: Some(receipt_data.to_string()),
                },
            ))
        }
        Err(e) => {
...
fn fetch_nonce_from_entry_point(
    ...
    use alloy_sol_types::*;
    use hyperware_process_lib::eth::{TransactionInput, TransactionRequest};
    ...
    match provider.call(nonce_tx_req, None) {
        ...
        Err(e) => Err(format!("Failed to fetch nonce: {}", e)),
    }
}
...
fn fetch_dynamic_gas_prices(provider: &Provider) -> Result<(u128, u128), String> {
    ...
    match provider.get_block_by_number(BlockNumberOrTag::Latest, false) {
        ...
        Ok(None) => Err("No latest block found".to_string()),
        Err(e) => Err(format!("Failed to get latest block: {}", e)),
    }
}
...
fn get_user_op_hash_for_estimation(
    ...
    match provider.call(tx_req, None) {
        Ok(bytes) => {
            info!(
                "Got UserOp hash from EntryPoint for estimation: 0x{}",
                hex::encode(&bytes)
            );
        ...
        Err(e) => Err(format!("Failed to call EntryPoint.getUserOpHash(): {}", e)),
    }
}
