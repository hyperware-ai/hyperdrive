...
use hyperware_process_lib::hyperwallet_client::types::{
    HyperwalletRequest, HyperwalletResponse, HyperwalletResponseData,
    OperationError, HandshakeStep, UnlockWalletRequest, UnlockWalletResponse,
    HandshakeRequest, Operation, 
};
use hyperware_process_lib::logging::{info, warn};
use hyperware_process_lib::Address;

pub fn handle_handshake_step(
    request: HandshakeRequest<HandshakeStep>,
    source: &Address,
    state: &mut HyperwalletState,
) -> HyperwalletResponse<HandshakeStep> {
    state.cleanup_expired_sessions();

    info!("Received handshake request {:?} from {:?}", request, source);
    
    let step = &request.step;
    
    match &step {
        HandshakeStep::ClientHello { client_version, client_name } => {
            handle_client_hello(client_version.clone(), client_name.clone(), state)
        }
        HandshakeStep::Register { required_operations, spending_limits } => {
            handle_register(required_operations.clone(), spending_limits.clone(), source, state)
        }
        _ => return HyperwalletResponse::error(OperationError::invalid_params("Invalid handshake step"))
    }
...
fn handle_client_hello(
...
) -> HyperwalletResponse<HandshakeStep> {  // ← Return specific type
    ...
    info!("Sending ServerWelcome with {} operations and {} chains", 
          supported_operations.len(), 1);

    HyperwalletResponse::success(HandshakeStep::ServerWelcome {
...
fn handle_register(
    ...
    info!("Processing Register for {} with {} operations", address, required_operations.len());
    ...
    HyperwalletResponse::success(HandshakeStep::Complete {
        session_id,
...
pub fn handle_unlock_wallet(
    ...
    match state.validate_session(&data.session_id) {
    ...
    let (wallet_address, key_storage) = match state.get_wallet(address, &data.wallet_id) {
        Some(wallet) => (wallet.address.clone(), wallet.key_storage.clone()),
        None => {
            return HyperwalletResponse::error(OperationError::wallet_not_found(&data.wallet_id));
        }
    };
    
    let result = match &key_storage {
        crate::state::KeyStorage::Encrypted(encrypted_data) => {
            match hyperware_process_lib::signer::LocalSigner::decrypt(encrypted_data, &data.password) {
                Ok(signer) => {
                    state.active_signers.insert(
                        (address.clone(), wallet_address.clone()), 
                        signer
                    );
                    
                    if let Err(e) = state.add_unlocked_wallet(&data.session_id, wallet_address.clone()) {
                        return HyperwalletResponse::error(OperationError::internal_error(&e));
                    }
                    
                    info!("Unlocked wallet {} for session {}", wallet_address, data.session_id);
                    
                    HyperwalletResponse::success(UnlockWalletResponse {
                        success: true,
                        wallet_id: data.wallet_id.clone(),
                        message: format!("Wallet {} unlocked successfully", wallet_address),
                    })
                }
                Err(_) => {
                    HyperwalletResponse::error(OperationError::authentication_failed("Invalid password"))
                }
            }
        }
        crate::state::KeyStorage::Decrypted(_) => {
            ...
            info!("Wallet {} was already unlocked, added to session {}", wallet_address, data.session_id);
    ...
    result.map(HyperwalletResponseData::UnlockWallet)
}
