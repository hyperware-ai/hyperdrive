...
fn init(_our: Address) {
    ...
    println!("Enabling dao-cacher provider mode...");

    let response = Request::to(("our", "dao-cacher", "hypermap-cacher", "sys"))
        .body(DaoCacherRequest::StartProviding)
        .send_and_await_response(5);

    match response {
        Ok(Ok(message)) => match message.body().try_into() {
            Ok(DaoCacherResponse::StartProviding(Ok(msg))) => {
                println!("✓ {}", msg);
            }
            Ok(DaoCacherResponse::StartProviding(Err(err))) => {
                println!("✗ Failed to enable dao-cacher provider mode: {}", err);
            }
            _ => {
